...
import com.example.url_blocker.media.data.LiveStreamConfig
 ...
 *
 * The player occupies the same composition slot in both orientations so
 * rotation never restarts the stream — portrait shows the selector over the
 * player, landscape becomes immersive fullscreen (handled by the hub).
 * State-driven UI: a connecting indicator while resolving/buffering, the
 * in-app "Live stream currently unavailable" card + Retry when nothing is
 * airing, the stream fails, or it can't be embedded.
 */
@Composable
fun LiveTab(
    isLandscape: Boolean,
    ...
    Box(modifier = modifier.fillMaxSize().background(MaterialTheme.colorScheme.background)) {
        ...
        // Single, stable player slot: never recreated on rotation. The live
        // broadcast video id is applied when resolution succeeds; while null
        // the player page idles under the connecting overlay.
        YoutubePlayer(
            videoId = videoId,
        ...
        if (!isLandscape) {
            // Portrait: selector + caption over the player (top / bottom).
