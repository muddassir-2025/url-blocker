...
## YouTube feed blocking in Chrome (pre-emptive video-title check)

Logcat filter for these tests: `adb logcat -s UrlBlockerService KeywordMatcher ContentExtractor`

Prerequisites: Accessibility Service enabled and "Display over other apps" granted.

1. Open `m.youtube.com` in Chrome and search a blocked keyword (e.g. "cleavage"). Every matching result card must be caught BEFORE any video is opened: `YOUTUBE_FEED_SIGNAL` / `FEED BLOCK MARKED ... cards=M` confirm the flow, and the full block overlay appears (close tabs + Home).
2. Deceptive-video case: a card whose TITLE is safe but whose channel name (or thumbnail caption) is suggestive must still be caught — `YOUTUBE_FEED_SCORE score=... reasons=[THUMBNAIL:...]` / `reasons=[CHANNEL:...]` confirm the weighted risk score fired (score >= 0.3).
3. Dedup: a text-blocked card set logs `FEED BLOCK MARKED` ONCE per distinct set (`cards=` titles) — repeat scans while the same cards are on screen must not re-log it. Event-driven tree scans are throttled to 1/250ms, so a `TYPE_WINDOW_CONTENT_CHANGED` storm must not jank the UI (`Skipped N frames` in logcat indicates a regression).
...
4. If a blocked video opens anyway (related rail, reopened tab), the watch-page full block must fire (`BLOCK DETECTED` + overlay) — the fallback when the feed check missed it.
5. Search pages are never blocked as a whole: searching a blocked word (e.g. strict-mode `cleavage`) on m.youtube.com must NOT trigger a full page block — the `/results?search_query=...` URL and the "<query> - YouTube" page title are the USER'S QUERY, not video content. Only individual result CARDS are checked (`YOUTUBE_FEED_SIGNAL` / `FEED BLOCK MARKED` with a card list). A blocked DOMAIN rule still applies to the host.
...
6. Keyword false-positive regression: with Strict Mode enabled, clean searches/videos using ambiguous everyday words (`dating`, `date`, `body`, `model`, `oral`, `facial`, `mature`, `webcam`, `dominance`, `submission`) must NOT be blocked — the strict list is a small curated set of discovery terms (bikini, lingerie, cleavage, ...) and these words are not in it.
...
2. Arabic pipeline: first run downloads BOTH editions (`eng-mustafakhattaba` — The Clear Quran — + `ara-quranindopak` IndoPak script, ~3.5 MB total one-time). If only English is cached, the verse screen still works and simply omits the Arabic line.
...
4. Media tab — **Subscriptions-style feed (aggregate)**: a channel avatar strip at the top (`All` · circular channel avatars — scraped from each channel page via `ChannelAvatarResolver` with an initials fallback · `+` to add). The feed below aggregates EVERY saved channel (cached data first — a `cached` hint shows — then a background refresh of all feeds merged newest-first; Safina Society is seeded by default; add via `+` — paste a `@handle`, channel URL or bare `UC…` id; remove a channel via the `✕` on its avatar chip with a confirmation dialog). The feed is split into a **Shorts** row (vertical cards with the title + view count overlaid on a bottom-scrimmed thumbnail and a play badge in the corner — classified as Shorts from the `#shorts` hashtag in the title OR the channel's `/shorts` tab scraped by `ShortsIdResolver`, so Shorts that omit the hashtag still appear; log `SHORTS_RESOLVED channelId=… count=…`) followed by the **Videos** list (YouTube-style rows: 16:9 thumbnail on the left with progress bar/Watched badge, title + `Channel · 1.2K views · 3d ago` on the right — view counts come from the feed's `<media:statistics views=…/>`). Section headers carry a small brand-colored accent bar. Tapping a channel avatar filters the feed to that channel; tapping again or tapping `All` restores everything. Offline first open → an in-app error card, no crash.
4b. **Watch progress**: play a video, let it run past ~10 s, then go back — the video's thumbnail now shows a thin red progress bar along its bottom edge. Finish the video (or reach ≥90%) and the card shows a dimmed thumbnail with a `✓ Watched` badge. Progress comes from the player's REAL position: logcat shows `IFRAME_PROGRESS 42s/315s` every ~5 s while playing (the JS reports `getCurrentTime()/getDuration()`), persisted by `WatchProgressStore` (SharedPreferences) on pause/end too — a video stopped halfway must show ~50% the next time you open the Media tab. Progress survives an app restart (per-video, keyed by video id).
...
6b. **Fullscreen BUTTON (fixed — was blank)**: the player's built-in fullscreen button used to blank the screen (audio kept playing) because `onShowCustomView` only rotated and never displayed the video surface the WebView handed over — the surface was orphaned. Now the custom view is hosted in a fullscreen overlay attached to the window (`FULLSCREEN_SHOW customView=…` / `FULLSCREEN_HIDE` logcat lines): tapping the button rotates to landscape, hides the bars, and shows the video; tapping the minimize/X returns to the in-page player (`FULLSCREEN_HIDE`) and the orientation restores. Test the button on a regular video AND on each Live channel; also test pausing the app while fullscreen (Home → return) — the custom view is torn down on pause so the player is never left blank.
7. Live tab: `[🕋 Makkah Live] [🕌 Madinah Live]` chips above the player. Each channel's CURRENT live broadcast video id is resolved at runtime from the official YouTube channel (`LiveStreamResolver` fetches `https://www.youtube.com/channel/<id>/live`), then played with the **same in-app IFrame player as regular videos** — no HLS/CDN. Resolution falls through the discovery tiers below, because YouTube serves different HTML variants of the same `/live` URL by region/network (the most reduced ones carry an EMPTY `ytInitialPlayerResponse` stub with no videoId and no live marker):
   - **Tier 1 — player-response live markers**: `"isLive":true` / `"isLiveNow":true` or `liveStreamability` inside the `/live` page's player response (upcoming broadcasts carry `isLiveNow:false` and must NOT resolve). Log: `LIVE_RESOLVED channelId=… videoId=…`.
   - **Tier 2 — canonical link**: the `/live` page's `<link rel="canonical">` names the broadcast on a `/live` URL (`…/watch?v=<id>`); on a channel page it names the channel (no id → correctly not live). Log: `LIVE_RESOLVED_VIA_CANONICAL channelId=… videoId=…`.
   - **Tier 3 — first videoId in the document**: the page's FIRST `videoId` anywhere in the HTML — reduced variants embed the airing broadcast's id even when the player response is an empty stub (on-device logs: `firstVideoId=Rs7St51oDDc` with `canonical=null`). Only runs when the page presented a player response. Log: `LIVE_RESOLVED_VIA_PAGE_ID channelId=… videoId=…`.
   - **Tier 4 — channel video data**: the channel's `/streams` page — the first video id it presents (always the channel's own videos). Log: `LIVE_RESOLVED_VIA_STREAMS channelId=… videoId=…`.
   - **Tier 5 — last known (persisted per channel)**: the per-channel cached id from `LiveStreamCacheStore` (SharedPreferences `live_stream_cache`, separate keys per Makkah/Madinah — never shared), used ONLY when every tier above failed and ONLY after its `/watch` page re-validates it (still live, correct channel, playable). A stale/ended id is never played. Logs: `LIVE_RESOLVED_VIA_CACHED channelId=… videoId=…` / `LIVE_CACHED_STALE …`.
   **MANDATORY VALIDATION (every accepted candidate, including the cached one)**: its `/watch` page must confirm the video is CURRENTLY LIVE (live markers in the player response), the candidate IS that page's own video, it belongs to the EXPECTED official channel (`ownerChannelId` — a Makkah video can never be accepted for Madinah or vice versa), and it is PLAYABLE (`playabilityStatus` OK). Any failed check rejects the candidate.
   **Restricted-Mode / DNS-filter detection**: a page whose player response carries `playabilityStatus.status = ERROR`, no live markers, and no canonical watch link is the signature CleanBrowsing-family DNS filters produce (they map `www.youtube.com` to Google's Restricted-Mode IPs, which suppress live streams server-side — the `/live` page then returns ~795 KB, `canonical=null`). Other statuses (LOGIN_REQUIRED, age/geo) are NOT reported as a filter block. The tab shows the specific `…DNS filter is blocking YouTube live streams (Restricted Mode)…` message and stops auto-retrying — retrying cannot help until YouTube is exempted in the filter (allowlist YouTube domains or disable its YouTube Restricted Mode enforcement).
   The `LIVE_PAGE` diagnostic line reports the page variant: `hasPlayerResponse` / `hasIsLiveMarker` / `hasLiveStreamability` / `canonical` / `firstVideoId` / `status` (the playability status — `ERROR` = Restricted Mode/filter).
   Sources: Makkah = قناة القرآن الكريم (`UCos52azQNBgW63_9uDJoPDA`, Masjid al-Haram), Madinah = قناة السنة النبوية (`UCROKYPep-UuODNwyipe6JMw`, Masjid an-Nabawi). **Makkah must show Masjid al-Haram and Madinah must show Masjid an-Nabawi — the previous Al Jazeera CDN URLs (`getaj.net/AJA/…`) and the Saudi CDN HLS URLs (`svs.itworkscdn.net/…`, which return HTML block pages in practice) are removed.** Only official Saudi channels are used, and NO live video id is hardcoded — the current broadcast is discovered at runtime, so a new broadcast never requires an app update. Only one stream loads at a time. States: `Connecting to live stream…` (RESOLVING), `Live stream temporarily unavailable. Retrying…` (RETRYING — auto-retries with 5 s → 10 s → 20 s → 40 s exponential backoff, bounded, so YouTube is never hammered), `Live · playing inside the app` when `IFRAME_STATE=1` (PLAYING), and the `Live stream currently unavailable. Please try again later.` card with a `Retry` (TEMPORARILY_UNAVAILABLE) after the bounded retries are exhausted or an embed error (`LIVE_EMBED_ERROR` / `LIVE_STREAM_ENDED`). Nothing ever redirects externally. Landscape → immersive fullscreen for the player. Sources: Makkah = قناة القرآن الكريم (`UCos52azQNBgW63_9uDJoPDA`, Masjid al-Haram), Madinah = قناة السنة النبوية (`UCROKYPep-UuODNwyipe6JMw`, Masjid an-Nabawi). **Makkah must show Masjid al-Haram and Madinah must show Masjid an-Nabawi — the previous Al Jazeera CDN URLs (`getaj.net/AJA/…`) and the Saudi CDN HLS URLs (`svs.itworkscdn.net/…`, which return HTML block pages in practice) are removed.** Only one stream loads at a time; a `Connecting to live stream…` indicator shows while resolving/buffering; the caption flips to `Live · playing inside the app` when `IFRAME_STATE=1`. If nothing is airing, resolution fails, or the embed errors (log `LIVE_NOT_ACTIVE` / `LIVE_EMBED_ERROR`), the in-app `Live stream currently unavailable. Please try again later.` card appears with a `Retry` (forces a fresh resolve). Nothing ever redirects externally. Landscape → immersive fullscreen for the player.
...
9. URL-blocker isolation: with the Accessibility Service active, opening the hub and playing Media/Live must NEVER trigger a block overlay — `UrlBlockerService` early-returns for its own package (`com.muddassir.clearview`) before the embedded-browser probe (line ~362 `if (currentForegroundPackage == OUR_PACKAGE) return`), and `GLOBAL_ACTION_HOME` only runs in blocking sequences for Chrome/Google/YouTube. The `Foreground package changed: com.muddassir.clearview -> com.motorola.launcher3` log lines are the service OBSERVING the user pressing Home, not the service causing it. Check logcat for the absence of `EMBEDDED_BROWSER_DETECTED` for the app's own package.
